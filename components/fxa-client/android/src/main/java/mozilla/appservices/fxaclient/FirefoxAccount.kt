...
class FirefoxAccount(handle: FxaHandle, persistCallback: PersistCallback?) : AutoCloseable {
     ...
     * returned account events, they will never been seen again.
     *
     * This performs network requests, and should not be used on the main thread.
     *
    ...
    /**
     * Ensure the current device is registered with the specified name and device type.
     * The Send Tab command is also registered with the server.
     * This method should be called once per "device lifetime", and since the Send Tab
     * command relies on Sync Keys, after the Sync Keys have been
     * obtained and called again if they change.
     *
     * This performs network requests, and should not be used on the main thread.
     */
    fun ensureDeviceRegistered(deviceType: Device.Type, name: String) {
        rustCall { e ->
            LibFxAFFI.INSTANCE.fxa_ensure_device_registered(this.handle.get(), deviceType.toNumber(), name, e)
        }
        this.tryPersistState()
    }

...
/// Ensures that our own device is registered and the send tab command has been registered.
#[no_mangle]
pub extern "C" fn fxa_ensure_device_registered(
    handle: u64,
    device_type: i32,
    name: FfiStr<'_>,
    error: &mut ExternError,
) {
    log::debug!("fxa_ensure_device_registered");
    ...
    ACCOUNTS.call_with_result_mut(error, handle, |fxa| {
        fxa.ensure_device_registered(device_type.into(), name.as_str())
    })
}
